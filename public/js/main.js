var css2 = "font-weight: bold";
console.log(
  "%cSi usted a entrado aqui es por que sabes del tema o ande aburrido en todo caso si usted sabes de javascript y node pongase en contracto con nosotros.",
  css2
);
...
if (window.location.pathname === "/servants") {
  ...
  // Get Select Buttom Clase
  const clasesList = [];

  // Se id to clase with map
  const idToClase = new Map()
    .set(0, "saber")
    .set(1, "shielder")
    .set(2, "archer")
    .set(3, "lancer")
  ...
  if(mobile()) {
    ...
    var tableScroll = new ScrollBooster({
      ...
      onUpdate: (state) => {
        ...
        if(offeset > viewheighoffset - 278) {
          toUp = true
          content.style.transform = `translate(
            0px,
            ${-viewheighoffset + 278}px
          )`;
        } else {
          toUp = false
